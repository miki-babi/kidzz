...
function getSkillLevel(value: number): {
    ...
    if (value >= 100) {
        return { level: 5, tier: 'GOLD', color: '#FFD700' };
    }

    if (value >= 80) {
        return { level: 4, tier: 'GOLD', color: '#FFD700' };
    }

    if (value >= 60) {
        return { level: 3, tier: 'SILVER', color: '#A78BFA' };
    }

    if (value >= 40) {
        return { level: 2, tier: 'BRONZE', color: '#E5A365' };
    }
...
function getTierBorder(tier: string): string {
    if (tier === 'GOLD') {
        return 'border-[#FFD700] shadow-[0_3px_0_#D4A800]';
    }

    if (tier === 'SILVER') {
        return 'border-[#A78BFA] shadow-[0_3px_0_#8B6AE0]';
    }
    ...
    return (
        <div className="flex flex-col items-center gap-3 rounded-2xl border-2 border-[#E5E5E5] bg-white p-5 shadow-[0_4px_0_#C4C4C4] transition-all active:translate-y-0.5">
            ...
            <div className="text-center">
                ...
                <span className="block text-[11px] font-black tracking-widest text-[#777777] uppercase">
                    {label}
                </span>
            </div>
        </div>
        ...
        <div className="rounded-2xl border-2 border-[#E5E5E5] bg-white p-5 shadow-[0_4px_0_#C4C4C4]">
            <div className="mb-4 flex items-center gap-3">
                ...
                <div className="flex-1">
                    ...
                    <span
                        className="mt-0.5 inline-block rounded-md px-2 py-0.5 text-[10px] font-black tracking-wider uppercase"
                        style={{
                ...
                <div className="flex items-center justify-center rounded-xl border-2 border-[#E5E5E5] bg-[#F7F7F8] px-3 py-1.5 shadow-[0_2px_0_#C4C4C4]">
            ...
            <div className="h-5 w-full overflow-hidden rounded-full border-2 border-[#E5E5E5] bg-[#E5E5E5]">
                <div
                    className={`h-full rounded-full transition-all duration-500 ${color}`}
                    style={{
                        width: `${value}%`,
                        boxShadow:
                            'inset 0 -2px 0 rgba(0,0,0,0.15), inset 0 2px 0 rgba(255,255,255,0.4)',
                    }}
                />
            </div>
            ...
            {data.map((d, i) => {
                ...
                return (
                    ...
                    >
                        ...
                        <span className="text-[11px] font-black text-[#777777]">
                            {dayLabels[i]}
                        </span>
                    </div>
                );
    ...
    return (
        <div className="mt-4">
            ...
            <div className="relative mt-4">
                {/* Milestone goal line */}
                <div
                    className="absolute right-0 left-0 z-10 border-t-2 border-dashed border-[#FF4B4B]"
                    style={{ bottom: `${goalPercent}%` }}
                >
                    <span className="absolute -top-3 right-0 text-[10px] font-black tracking-wider text-[#FF4B4B] uppercase">
                        Daily Goal
                    </span>
                </div>
                ...
                <div className="flex h-32 items-end justify-between gap-2">
                    {data.map((day) => {
                        ...
                        return (
                            ...
                            >
                                <div className="relative flex h-24 w-full flex-col items-center justify-end">
                                    {day.minutes > 0 ? (
                                        <div className="relative w-full max-w-[36px]">
                                            {/* Base weight shadow */}
                                            <div
                                                className="absolute right-0.5 bottom-0 left-0.5 h-2 rounded-b-lg bg-neutral-400"
                                                style={{ height: '6px' }}
                                            />
                                            {/* Main bar */}
                                            <div
                                                className={`relative w-full overflow-hidden rounded-t-lg border-2 border-[#E5E5E5] transition-all duration-300 ${
                                                    hitGoal
                                                        ? 'bg-[#FF9600]'
                                                        : 'bg-[#58CC02]'
                                                }`}
                                            ...
                                            >
                                                {/* Shiny highlight */}
                                                {hitGoal && (
                                                    <div className="absolute top-1 right-1 left-1 h-1 rounded-full bg-white/40" />
                                                )}
                                            </div>
                                        </div>
                                    ) : (
                                        <div className="h-full w-full max-w-[36px] rounded-t-lg border-2 border-[#E5E5E5] bg-neutral-100" />
                                    )}
                                </div>
                                <span className="text-[11px] font-black text-[#777777]">
                                    {day.day}
...
export default function ParentsIndex() {
    ...
    const skillTopics = [
        {
            ...
            value: p.basicInformation,
            color: 'bg-emerald-500',
            icon: User,
        },
        {
            title: 'Professional Evaluation',
            value: p.professionalEvaluation,
            color: 'bg-sky-500',
            icon: Stethoscope,
        },
        {
        ...
        },
        {
            ...
            value: p.speakingSkills,
            color: 'bg-yellow-500',
            icon: MessageCircle,
        },
        {
            title: 'Understanding Language',
            value: p.understandingLanguage,
            color: 'bg-teal-400',
            icon: Heart,
        },
        {
            title: 'Learning Concepts',
            value: p.learningConcepts,
            color: 'bg-indigo-400',
            icon: Puzzle,
        },
        {
            title: 'Thinking Skills',
            value: p.thinkingSkills,
            color: 'bg-amber-500',
            icon: Brain,
        },
        {
            title: 'Social Play Skills',
            value: p.socialPlaySkills,
            color: 'bg-pink-400',
            icon: Users,
        },
        {
            title: 'Interests Engagement',
            value: p.interestsEngagement,
            color: 'bg-cyan-400',
            icon: Lightbulb,
        },
    ];
    ...
    return (
        <AppDashboardLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="mx-auto flex w-full max-w-5xl flex-1 flex-col gap-6 px-4 pt-2 pb-8">
                {/* Page Header */}
                <div className="border-b-2 border-[#E5E5E5] pb-4">
                    <h1 className="text-4xl font-black tracking-tight text-[#3C3C3C]">
                        Child Progress
                    </h1>
                ...
                <div className="flex-1 space-y-6">
                    ...
                    <div className="rounded-2xl border-2 border-[#E5E5E5] bg-white p-6 shadow-[0_4px_0_#C4C4C4]">
                        ...
                        <p className="mb-2 text-xs font-bold text-[#777777]">
