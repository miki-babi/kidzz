...
export function MatchPairs({
    ...
    function renderFace(it: MatchItem, big = false) {
        ...
        return (
            <span className={big ? 'text-4xl' : 'text-3xl'}>{it.label}</span>
        );
    }
    ...
    const tapBottom = useCallback(
        (it: MatchItem) => {
            if (matched.includes(it.key) || won || checkResult !== 'idle') {
                return;
            }
            ...
            if (selected === it.key) {
                ...
                if (next.length >= items.length) {
                    ...
                    router.post(`/games/${gameId}/result`, {
                        score: 100,
                        duration,
                    });
